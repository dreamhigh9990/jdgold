...
<footer class="main-footer">
    ...
    <strong>Copyright &copy; Bhavani LLC.</strong>
</footer>
</div>
<!-- /.control-sidebar -->
...
//                    $(".box-body i.fa.fa-refresh.fa-spin").remove();
    ...
    function initAjaxSelect2wTag($selector,$source_url)
    {   //KAJAL 27102022 - for new entry option tag
        $selector.select2({
            placeholder: " --Select-- ",
            allowClear: true,
            width:"100%",
            ...
            insertTag:function(data,tag){                
                data.push({
                    id:-1, text: tag.text, newOption: true
                });
            },
    ...
    function setSelect2Valuemanual($selector,$id,$name)
    {
             $selector.empty().append($('<option/>').val($id).text($name)).val($id).trigger("change");
        //   $selector.empty().append($('<option/>').val('').text('--select--')).val('').trigger("change");
