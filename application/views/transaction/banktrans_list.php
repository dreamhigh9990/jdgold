                    ...
                    <div class="col-md-12">
                        ...
                        <form id="form_print_multiple_transaction" class="" action="<?= base_url('transaction/print_multiple_transaction') ?>" method="post" enctype="multipart/form-data"  target="_blank" >
                            ...
                            <table class="table table-striped table-bordered transaction-table" id="transaction-table">
                                <thead>
                                    <tr>
                                        <th>Action&nbsp;&nbsp;<input type="checkbox" id="check_all" ></th>
                                        <th>Date</th>
                                        <th>Client</th>
                                        <th>Cash A/c</th>
                                        <th>Cash Amount</th>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {        
        ...
        table = $('.transaction-table').DataTable({
            ...
            "order": [],
            "ajax": {
                "url": "<?= base_url('transaction/banktransaction_datatable')?>",
                "type": "POST",
                "data": function (d) {
            ...
            footerCallback: function (row, data, start, end, display) {
                var api = this.api();
                cashTotal = api.column(4, { page: 'current' }).data().reduce(function (a, b) {return parseFloat(a) + parseFloat(b);}, 0);
                $(api.column(4).footer()).html(cashTotal.toFixed(2));
                swipTotal = api.column(7, { page: 'current' }).data().reduce(function (a, b) {return parseFloat(a) + parseFloat(b);}, 0);
                $(api.column(7).footer()).html(swipTotal.toFixed(2));
                commTotal = api.column(9, { page: 'current' }).data().reduce(function (a, b) {return parseFloat(a) + parseFloat(b);}, 0);
                $(api.column(9).footer()).html(commTotal.toFixed(2));
                cusCommTotal = api.column(10, { page: 'current' }).data().reduce(function (a, b) {return parseFloat(a) + parseFloat(b);}, 0);
                $(api.column(10).footer()).html(cusCommTotal.toFixed(2));
