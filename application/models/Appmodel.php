...
class AppModel extends CI_Model
{
    ...
    function getAllowAccountIDS($user_id)
    {
        $array = array();
        $sql = "SELECT * FROM user_allow_accounts WHERE user_id='" . $user_id . "'";
        $rows = $this->db->query($sql)->result();
        $i = 0;
        foreach ($rows as $row) {
            //$array[$i] = $row->website_module_id;
            $array[$i] = $row->allowed_account_id;
            $i++;
        }
        return $array;
    }

    function getAllowUserIDS($user_id)
    {
        $array = array();
        $sql = "SELECT * FROM user_allow_user_view WHERE user_id='" . $user_id . "'";
        $rows = $this->db->query($sql)->result();
        $i = 0;
        foreach ($rows as $row) {
            //$array[$i] = $row->website_module_id;
            $array[$i] = $row->user_allow_user_id;
            $i++;
        }
        return $array;
    }

    function getAllowAccountIDSView($user_id)
    {
        $array = array();
        $sql = "SELECT * FROM user_allow_accounts_view WHERE user_id='" . $user_id . "'";
        $rows = $this->db->query($sql)->result();
        $i = 0;
        foreach ($rows as $row) {
            //$array[$i] = $row->website_module_id;
