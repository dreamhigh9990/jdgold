...
//                                        }
	...
	function get_account_balance($account_id, $to_date = '', $site_id = '')
	{
        ...
        $user_curr_id = $this->crud->getFromSQL("SELECT currency_id FROM account WHERE account_id=" . $account_id);
        $from_curr_id = $this->crud->getFromSQL("SELECT currency_id FROM transaction_entry WHERE from_account_id=" . $account_id);
        $to_curr_id = $this->crud->getFromSQL("SELECT currency_id FROM transaction_entry WHERE to_account_id=" . $account_id);

        $user_currency_id = $user_curr_id[0]->currency_id;
        $from_currency_id = $from_curr_id[0]->currency_id;
        $to_currency_id = $to_curr_id[0]->currency_id;
		...
		//Start : /*-------- Payment ------*//*------- Receipt -------*//*-------- Contra ------*//*------- Journal ------*/
        $account_query_arr = $this->crud->getFromSQL("SELECT * FROM transaction_entry WHERE (from_account_id=" . $account_id . " OR to_account_id=" . $account_id . ") AND transaction_date <= '" . $to_date . "'");

	    foreach ($account_query_arr as $list_row) {
			//We have ignored "transaction_type"
			if($list_row->from_account_id == $account_id && $list_row->currency_id == $user_currency_id) {
			  $credit_total += $list_row->amount;
			}
			else if($list_row->from_account_id == $account_id && $list_row->base_currency_id == $user_currency_id) {
			  $credit_total += $list_row->base_currency_amount;
			}
			else if($list_row->to_account_id == $account_id && $list_row->currency_id == $user_currency_id) {
			  $debit_total += $list_row->amount;
			}
			else if($list_row->to_account_id == $account_id && $list_row->base_currency_id == $user_currency_id) {
			  $debit_total += $list_row->base_currency_amount;
			}
		}
		//End : /*-------- Payment ------*//*------- Receipt -------*//*-------- Contra ------*//*------- Journal ------*/
		...
		$acc_balance = $credit_total - $debit_total;
		// echo "Total: ". $acc_balance.' ('.$credit_total . '-' . $debit_total.')<br/><br/>';
