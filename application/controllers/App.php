...
//        echo $this->db->last_query(); exit;
    ...
    function account_select2_source($arg = null) {
		...
		$all_allow_account_arr = $this->db->select('website_module_id')->where(['user_id' => $user_id,'website_module_id'=>MODULE_ALLOWED_ACCOUNTS])->get('user_roles')->row_array();
		
		$all_allow_account = $all_allow_account_arr['website_module_id'];
		
		$allowed_account_ids_comma = array();
		if(empty($all_allow_account)){
			$allow_account_arr = $this->db->select('allowed_account_id')->where(['user_id' => $user_id])->get('user_allow_accounts')->result_array();
			$user_by_account_id = $this->db->select('account_id')->where(['user_created_by' => $user_id])->get('account')->result_array();
			...
			$user_by_account_ids_comma = array();
			foreach ($user_by_account_id as $value1) {
				$user_by_account_ids_comma[] = $value1['account_id'];
			}
			
			$user_by_allowed_account_id = array_merge($user_by_account_ids_comma,$allowed_account_ids_comma);
			if(empty($user_by_allowed_account_id)){
				$user_by_allowed_account_id[] = '0';
			}
		}
		...
		if (!empty($user_by_allowed_account_id)) {
			$this->db->where_in('a.account_id',$user_by_allowed_account_id);
		}
		if (!empty($search)) {
			$this->db->group_start();
	...
	function account_select2_source_view($arg = null) {
        $search = isset($_GET['q']) ? $_GET['q'] : '';
        $page = isset($_GET['page']) ? $_GET['page'] : 1;
		$user_id = $this->session->userdata('login_user_id');
		...
		$this->db->join("city c","c.city_id=a.account_city","left");
		if (!empty($allowed_account_ids_comma)) {
			$this->db->where_in('a.account_id',$allowed_account_ids_comma);
		}
		if (!empty($search)) {
			$this->db->group_start();
			$this->db->like("a.account_name", $search);
	...
	function ib_account_select2_source($arg = null) {
        $search = isset($_GET['q']) ? $_GET['q'] : '';
        $page = isset($_GET['page']) ? $_GET['page'] : 1;
        if (is_numeric($arg)) {
	...
	function currency_select2_source() {
		...
		$resultCount = 10;
		$offset = ($page - 1) * $resultCount;
		$this->db->select("id,currency_name");
		...
		$results = array(
				...
				"total_count" => $this->count_select2_data('currency_master', 'id', 'currency_name', $search),
		);
	...
	function cash_only_account_select2_source() {
		//KAJAL 19/10/2022 20:05
		$search = isset($_GET['q']) ? $_GET['q'] : '';
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$where_array = array(CASH_IN_HAND_ACC_GROUP_ID);
		$select2_data = array();
		$resultCount = 10;
		$offset = ($page - 1) * $resultCount;
		$this->db->select("account_id,account_name");
	...
	function swipe_machine_source() {
		//KAJAL 25/10/2022
		$search = isset($_GET['q']) ? $_GET['q'] : '';
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$select2_data = array();
		$this->db->select("machine_id,machine_name");
		$this->db->from("swipe_machines");
		...
		$results = array(
				...
				"total_count" => $this->count_select2_data('swipe_machines', 'machine_id', 'machine_name', $search)
		);
	...
	function set_lb_account_select2_val_by_id($id){
		...
		if ($query->num_rows() > 0) {
				echo json_encode(array('success' => true, 'id' => $query->row()->account_id, 'text' => $query->row()->account_name ));
				exit();
		}
		echo json_encode(array('success' => true, 'id' => '', 'text' => '--select--'));
		exit();
	...
	function set_responsible_account_select2_val_by_id($id){
		$this->db->select('a.account_id,a.account_name');
		$this->db->from("account a");
		$this->db->where('a.account_id', $id);
		$this->db->limit(1);
...
function set_agent_select2_val_by_id($id){
	...
	function set_bank_account_select2_val_by_id($id){
		//KAJAL 25/10/2022
		$this->db->select('a.account_id,a.account_name,c.city_name,a.account_gst_no');
		$this->db->from("account a");
		$this->db->join("city c","c.city_id=a.account_city","left");
	...
	function set_swipe_machine_val_by_id ($id){
		//KAJAL 26/10/2022
		$this->db->select('m.machine_id, m.machine_name');
		$this->db->from("swipe_machines m");
		// $this->db->join("city c","c.city_id=a.account_city","left");
		...
		if ($query->num_rows() > 0) {
				echo json_encode(array('success' => true, 'id' => $query->row()->machine_id, 'text' => $query->row()->machine_name));
				exit();
		}
		echo json_encode(array('success' => true, 'id' => '', 'text' => '--select--'));
		exit();
	...
	function mobile_clinet_from_transaction($num){
		//KAJAL 26/10/2022
		$rec = $this->crud->getFromSQL("SELECT account_id,account_name FROM account WHERE INSTR(account_mobile_numbers,'$num')>0;");
		if(sizeof($rec)>0){
			foreach($rec as $key=>$value){
				echo json_encode(array('success' => true, 'id' => $value->account_id, 'text' => $value->account_name));
				break;
			}
		} else {
				echo json_encode([]);
	...
	function bank_transaction_details($client_id){
		//KAJAL 26/10/2022
		$rec = $this->crud->getFromSQL("SELECT *, (SELECT a1.account_mobile_numbers FROM account a1 WHERE a1.account_id=x.from_account_id) from_mobile_numbers, IFNULL((SELECT a1.account_name FROM account a1 WHERE a1.account_id=x.from_account_id),'') from_name, IFNULL((SELECT a2.account_name FROM account a2 WHERE a2.account_id=x.to_account_id),'') to_name FROM transaction_entry x WHERE x.tag = (SELECT tag FROM transaction_entry t WHERE t.from_account_id=$client_id OR t.to_account_id=$client_id ORDER BY t.created_at DESC LIMIT 1) AND CHAR_LENGTH(x.tag)=13 LIMIT 4;");
		echo json_encode(
			[
				'transaction' => sizeof($rec)>0?$rec:[], 
				'config' => [
					'agent_commission_id'=>AGENT_COMMISSION_ACCOUNT_ID, 
					'bank_commission_id'=>BANK_COMMISSION_ACCOUNT_ID 
					] 
			]
		);
	}
	
	function get_client_images($client_id){
		//KAJAL 28/10/2022
		$files=[];
		if(file_exists(FCPATH . ACCOUNT_IMAGES . $client_id)){
			$available_images = glob(FCPATH . ACCOUNT_IMAGES . $client_id . '/*.*');
			foreach ($available_images as $key => $file) {
				array_push($files, base_url(ACCOUNT_IMAGES . "$client_id/" .basename($file)));
			}
...
//                    $text = $row->item_name. ' | ' .number_format((float) $row->sales_rate_val, 2, '.', ''). ' | ' .$closing_stock;
    ...
    function set_user_id_select2_multi_val_by_id($ids){
        $myArray = explode(',', $ids);
        $this->get_select2_text_by_id_multiple('user', 'user_id', 'user_name', $myArray);
    }
    ...
    function set_currency_account_select2_val_by_id($currency_id){
		$this->db->select('id,currency_name');
        $this->db->from("currency_master");
        ...
        if ($query->num_rows() > 0) {
            echo json_encode(array('success' => true, 'id' => $query->row()->id, 'text' => $query->row()->currency_name));
            exit();
        }
        echo json_encode(array('success' => true, 'id' => '', 'text' => '--select--'));
        exit();
    }
    function set_currency_account_select2_val_by_acc_id($acc_id){
        console.log($acc_id);
        $query = $this->crud->getFromSQL("SELECT c.currency_name as currency_name FROM currency_master as c LEFT JOIN account as a ON a.currency_id = c.id  WHERE a.account_id = " . $acc_id . "");
        if ($query->num_rows() > 0) {
            echo json_encode(array('success' => true, 'id' => $query->row()->id, 'text' => $query->row()->currency_name));
            exit();
        }
...
//        echo $this->db->last_query(); exit;
	...
	function our_bank_label_select2_source(){
        $search = isset($_GET['q']) ? $_GET['q'] : '';
        $page = isset($_GET['page']) ? $_GET['page'] : 1;
        $where = array('account_group_id'=>21); //Bank
        $results = array(
                "results" => $this->get_select2_data('account', 'account_id', 'account_name', $search, $page, $where),
                "total_count" => $this->count_select2_data('account', 'account_id', 'account_name', $search, $where),
        );
	...
	function our_agent_label_select2_source(){
		//KAJAL
		$search = isset($_GET['q']) ? $_GET['q'] : '';
		$page = isset($_GET['page']) ? $_GET['page'] : 1;
		$where = array('account_group_id'=>58); //Agent
		$list = [];
		array_push($list, ['id'=>-1,'text'=>'',"selected"=>true]);
		$raw = $this->get_select2_data('account', 'account_id', 'account_name', $search, $page, $where);
		foreach($raw as $item){
		...
		// var_dump($list);
		$results = array(
						"results" => $list, //$this->get_select2_data('account', 'account_id', 'account_name', $search, $page, $where),
						"total_count" => $this->count_select2_data('account', 'account_id', 'account_name', $search, $where),
		);
    ...
    {
        $this->db->select("id,currency_name,multiplier");
        $this->db->from("currency_master");
        $this->db->where('id', $currency_id);
    ...
    function getMultiplier(){
        $where_array = array(
            "currency_id" => $this->input->post('current_id'),
            "currency_multiplied_id" => $this->input->post('base_current_id'),
        );
        $multiplier = $this->crud->get_column_value_by_id('currency_rate','multiplier',$where_array);
