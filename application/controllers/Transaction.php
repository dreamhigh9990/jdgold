...
class Transaction extends CI_Controller {
    ...
    function update_is_checked()
    {
        ...
        $post_data =array();
        $post_data['is_checked'] = $is_checked;
        $where_array['transaction_id'] = $transaction_id;
    ...
    function update_lb()
    {
        ...
        $lb_account_id=$_POST['lb_account_id'];
        $transaction_id=$_POST['transaction_id'];
        $post_data =array();
        $post_data['ib_account_id'] = $lb_account_id;
        $where_array['transaction_id'] = $transaction_id;
        $account_id['account_id'] = $this->crud->get_column_value_by_id("transaction_entry","accountno_account_id",array('transaction_id' => $transaction_id));
        $account['ib_account_id'] = $lb_account_id;
        $this->crud->update('account', $account, $account_id);//UPDATE account SET ib_account_id = ".$post_data['ib_account_id']
        //WHERE id = ".$account_ib_account_id;
        $result = $this->crud->update('transaction_entry', $post_data, $where_array);
        exit();
        
    }

    function update_responsible_account_id() {
        $previous_responsible_account = null;
        $responsible_account_id=$_POST['responsible_account_id'];
        $transaction_id=$_POST['transaction_id'];
        $post_data =array();
        $post_data['responsible_account_id'] = $responsible_account_id;
        $where_array['transaction_id'] = $transaction_id;
        

        $stringQuery = "transaction_id,currency_id, transaction_date, trans_time, account_id, from_account_id, to_account_id, transaction_type, amount, created_at, created_by, updated_at, transaction_id_ref, responsible_account_id";
        $getTransactionResult = $this->crud->getFromSQL("SELECT ".$stringQuery." FROM transaction_entry WHERE transaction_id =" . $transaction_id);
        $curr_id = $this->crud->get_column_value_by_id("account","currency_id",array("account_id" => $responsible_account_id));
        // $multiplier = $this->crud->get_column_value_by_id("currency_rate","multiplier",array("currency_id" => $getTransactionResult[0]->currency_id,"currency_multiplied_id" => $curr_id));
        $multiplier = $this->crud->get_column_value_by_id("currency_rate","multiplier",array("currency_multiplied_id" => $curr_id));
        
        $base_curr_amount = $getTransactionResult[0]->amount * $multiplier;
 
        // Update Multiplier
        $post_data['multiplier'] = $multiplier;
 

        if(count($getTransactionResult) > 0){
            foreach ($getTransactionResult as $keyOfQ1 )
            {
                $from_account_id = (int) $keyOfQ1->from_account_id;//$to_account_id   = (int) $keyOfQ1->to_account_id;
                if($from_account_id == 458)
                {
                    $post_data['account_id'] = $responsible_account_id;
                    $post_data['to_account_id'] = $responsible_account_id;
                    $post_data['ib_account_id'] = $responsible_account_id;
                    if(isset($curr_id) && !empty($curr_id))
                    {
                        $post_data['base_currency_id'] = $curr_id;
                        if($getTransactionResult[0]->currency_id == $curr_id)
                        {
                            $post_data['base_currency_amount'] = $getTransactionResult[0]->amount;
                        }else{
                            $post_data['base_currency_amount'] = $base_curr_amount;
                ...
                {
                    $post_data['account_id'] = $responsible_account_id;
                    $post_data['from_account_id'] = $responsible_account_id;
                    $post_data['ib_account_id'] = $responsible_account_id;
                    if(isset($curr_id) && !empty($curr_id))
                    {
                        $post_data['base_currency_id'] = $curr_id;
            ...
            $getTransactionResult = $getTransactionResult[0];
            $previous_responsible_account = $getTransactionResult->responsible_account_id;

            $getResponsibleTransactionResult = $this->crud->getFromSQL("SELECT ".$stringQuery." FROM transaction_entry WHERE transaction_id_ref =" . $transaction_id . " and responsible_active = 1");

            // if(!empty($getResponsibleTransactionResult) && count($getResponsibleTransactionResult) > 0){
            //     foreach ($getResponsibleTransactionResult as $key) {
            //         if((int) $key->transaction_type === 1 || (int) $key->transaction_type === 2){
            //             $from_account = $key->to_account_id;
            //             $to_account = $key->from_account_id;
            //             $transaction_type = (int) $key->transaction_type===1 ? 2 : 1;

            //             $tran_data['transaction_date'] = date('Y-m-d');
            //             $tran_data['from_account_id'] = $from_account;
            //             $tran_data['to_account_id'] = $to_account;
            //             $tran_data['transaction_type'] = $transaction_type;
            //             $tran_data['amount'] = $key->amount;
            //             $tran_data['created_at'] = $this->now_time;
            //             $tran_data['created_by'] = $this->logged_in_id;
            //             $tran_data['transaction_id_ref'] = $key->transaction_id_ref;
            //             //$this->crud->insert('transaction_entry',$tran_data);



            //             // Update
            //             $post_data_inner =array();
            //             $post_data_inner['responsible_active'] = 0;
            //             $where_array_inner['transaction_id'] = $key->transaction_id;
            //             $result = $this->crud->update('transaction_entry', $post_data_inner, $where_array_inner);
            //             // Update
            //         }
            //     }
            // }
            ...
            // get pre responsible_account
            // for ($i=0; $i < 2; $i++) { 
            //     if((int) $getTransactionResult->transaction_type === 1 || (int) $getTransactionResult->transaction_type === 2){
            //         $tran_data = array();
            //         $from_account = null;
            //         $to_account = null;
            //         if($i===0){
            //             if(!$previous_responsible_account){
            //                 $from_account = $getTransactionResult->to_account_id;
            //                 $to_account = $getTransactionResult->from_account_id;
            //                 $transaction_type = (int) $getTransactionResult->transaction_type===1 ? 2 : 1;
            //             }
            //         }else{
            //             if((int) $getTransactionResult->transaction_type === 1){
            //                 $from_account = $previous_responsible_account ? $previous_responsible_account : $getTransactionResult->to_account_id;
            //                 $to_account = $responsible_account_id;
            //             }else if((int) $getTransactionResult->transaction_type === 2){
            //                 $from_account = $responsible_account_id;
            //                 $to_account = $previous_responsible_account ? $previous_responsible_account : $getTransactionResult->from_account_id;
            //             }
            //             $transaction_type = (int) $getTransactionResult->transaction_type;
            //         }
            ...
            //             $tran_data['to_account_id'] = $to_account;
            //             $tran_data['transaction_type'] = $transaction_type;
            //             $tran_data['amount'] = $getTransactionResult->amount;
            //             $tran_data['created_at'] = $this->now_time;
            //             $tran_data['created_by'] = $this->logged_in_id;
            //             $tran_data['transaction_id_ref'] = $transaction_id;
            //             $tran_data['responsible_active'] = 1;
            //             //$this->crud->insert('transaction_entry',$tran_data);
            //         }
            //     }
            // }
            ...
            $output = array(
                "status" => true,
                "data" => "Responsible account changed successfully.",
                "multiplier" =>$multiplier,
            );
        }else{
            $output = array(
                "status" => false,
                "data" => "Something wen't wrong, please try again later.",
            );    
        }
    ...
    function update_multiplier()
    {
        ...
        $amounts = $this->db->select('amount')->where('transaction_id',$transaction_id)->get('transaction_entry')->row_array();
        
        $amount = $amounts['amount'];
        
        $post_data =array();
        
        $base_currency_amount = $amount * $multiplier;

        $post_data['multiplier'] = $multiplier;
        
        $post_data['base_currency_amount'] = $base_currency_amount;
    ...
    function mbtrans($transaction_id = '') {
        $data = array();

        $user_default_cash_acc_id=$this->crud->get_column_value_by_id("user","user_default_cash_acc_id",array('user_id' => $_SESSION['login_user_id']));


        $data['user_default_cash_acc_id']=$user_default_cash_acc_id;


        $transaction_date = $this->crud->get_column_value_by_id('company_settings','setting_value',array('company_id' => $_SESSION['login_user_id'],'setting_key' => 'payment_date'));
        ...
        $base_currency_id = $this->crud->get_column_value_by_id('account','currency_id',array('account_id' => $user_default_cash_acc_id));
        if(!empty($base_currency_id)) {
            $data['base_currency_id'] = $base_currency_id;
        }
        
        if (isset($transaction_id) && !empty($transaction_id)) {
            if($this->applib->have_access_role(MODULE_MBTRANS_ID,"edit")) {
                ...
                $currency_n = $data['transaction_data'];
                $currency = $this->db->select('currency_name,id')->where('id',$currency_n->currency_id)->get('currency_master')->row_array();
                $data['currency'] = $currency; 
                $base_currency = $this->db->select('currency_name,id')->where('id',$currency_n->base_currency_id)->get('currency_master')->row_array();
                $data['base_currency'] = $base_currency; 
                // echo"<pre>";
                // print_r($currency);die;
                $data['currecy_name'] = $currency[0];
                set_page('transaction/mbtrans', $data);
            } else {
            ...
            if($this->applib->have_access_role(MODULE_MBTRANS_ID,"add")) {
    ...
    function mbtrans_datatable()
    {
        $config['table'] = 'transaction_entry tr';
		$config['select'] = 'tr.*, tr.transaction_date AS tr_date, a.account_name as opp_acc_name, cr.currency_name as currency_name';
		$config['column_order'] = array(null,'tr_date','opp_acc_name','opp_acc_name','tr.amount', 'cr.currency_name');
		// $config['column_search'] = array('pa.account_group_name','a.account_group_name');
		$config['joins'][] = array('join_table' => 'account a', 'join_by' => 'a.account_id = tr.to_account_id', 'join_type' => 'left');
		$config['joins'][] = array('join_table' => 'currency_master cr', 'join_by' => 'cr.id = tr.currency_id', 'join_type' => 'left');
		
        $this->load->library('datatables', $config, 'datatable');
		$lists = $this->datatable->get_datatables();
		$data = array();
		$isEdit = $this->applib->have_access_role(MODULE_MBTRANS_ID, "edit");
		$isDelete = $this->applib->have_access_role(MODULE_MBTRANS_ID, "delete");
        ...
        foreach ($lists as $list) {
			...
			$action = '';
			if($isEdit) {
				// $action .= '<form id="edit_' . $list->transaction_id . '" method="post" action="' . base_url() . 'master/account_group" class="pull-left">
				// 	<input type="hidden" name="id" id="id" value="' . $list->transaction_id . '">
				// 	<a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $list->transaction_id . '\').submit();" title="Edit Account Group"><i class="fa fa-edit"></i></a>
				// 	</form>';
                $action = '<a href="' . base_url("transaction/mbtrans/" . $list->transaction_id) . '" class="edit_button btn-primary btn-xs"><i class="fa fa-pen"></i></a>';  
			}
			if($isDelete) {
                    $action .= ' <a href="javascript:void(0);" class="delete_transaction btn-danger btn-xs" data-href="' . base_url('transaction/delete_transaction/' . $list->transaction_id) . '"><i class="fa fa-trash"></i></a>';

					// $action .= ' &nbsp; <a href="javascript:void(0);" class="delete_button btn-danger btn-xs" data-href="' . base_url('master/delete_account_group/' . $list->transaction_id) . '"><i class="fa fa-trash"></i></a>';
                    // $action.=' &nbsp; <a class="delete_button btn-danger btn-xs" href="javascript:void(0);" data-href="' . base_url('transaction/delete_transaction/' . $list->transaction_id) . '"><i class="fa fa-trash"></i></a>';
				
			}
			
            $from_account_name= $this->crud->get_val_by_id('account',$list->from_account_id,'account_id','account_name');

			
			$row[] = $action;
			$row[] = $list->tr_date;
			$row[] = $from_account_name;
			$row[] = $list->opp_acc_name;
            $row[] = $list->amount;
    ...
    function mbtrans_list()
    {
        if($this->applib->have_access_role(MODULE_MBTRANS_ID,"view")) {
            $data = array();
            $data['transaction_type'] = 1;
            set_page('transaction/mbtrans_list', $data);
        } else {
            $this->session->set_flashdata('success', false);
            $this->session->set_flashdata('message', 'You have not permission to access this page.');
            redirect('/');
    ...
    function save_mbtransaction() {
        $post_data = $this->input->post();
    //    echo "<pre>"; print_r($post_data); exit;
        $checked_items = array();
        $checked_amt_data = array();

        $transaction_type = isset($post_data['transaction_type']) ? (int)$post_data['transaction_type'] : null;

        if($transaction_type===1){
            if(isset($post_data['from_account_id']) && $post_data['to_account_id']){
                $from_account_id = $post_data['from_account_id'];
                $to_account_id = $post_data['to_account_id'];
                $post_data['from_account_id'] = $to_account_id;
                $post_data['to_account_id'] = $from_account_id;
            }
        }
        ...
        if(isset($post_data['btndebit'])){
            // $checked_items = $post_data['btndebit'];
            unset($post_data['btndebit']);
        }
        if (isset($post_data['transaction_id']) && !empty($post_data['transaction_id'])) {
            ...
            $post_data['currency_id'] = $post_data['currency_id'];
            $post_data['multiplier'] = $post_data['multiplier'];
            //$post_data['select_user_default_cash_acc_id'] = $post_data['select_user_default_cash_acc_id'];
            $post_data['base_currency_amount'] = $post_data['base_currency_amount'];
...
//            echo $this->db->last_query(); exit;
    ...
    function banktransaction_datatable() {        
        $query = "SELECT t.*,
		IFNULL((SELECT account_name FROM account WHERE account.account_id=t.from_account_id),'') from_account_name, 
        IFNULL((SELECT account_name FROM account WHERE account.account_id=t.to_account_id),'') to_account_name, 
        sm.machine_name swipe_machine_name
        FROM `transaction_entry` t         
        INNER JOIN swipe_machines sm ON sm.machine_id=t.mach_id
        WHERE t.tag in (
                    SELECT tag FROM `transaction_entry` WHERE CHAR_LENGTH(tag) = 13
                    GROUP BY tag HAVING COUNT(transaction_id)>=2) ORDER by t.transaction_date DESC, t.tag";

        $list = $this->crud->getFromSQL($query);
        
        // PROCESS
        $trans_records = [];
        foreach($list as $trans){
            if(!isset($trans_records[$trans->tag])){
                $trans_records[$trans->tag] = ['tag'=>$trans->tag];                 
                $trans_records[$trans->tag]['transaction_id']=$trans->transaction_id;
                $trans_records[$trans->tag]['transaction_date']=$trans->transaction_date;
                $trans_records[$trans->tag]['machine']=$trans->swipe_machine_name;
                $trans_records[$trans->tag]['machine_id']=$trans->mach_id;
            }
            
            switch($trans->transaction_type){
                case 1:
                    $trans_records[$trans->tag]['client']=$trans->to_account_name;
                    $trans_records[$trans->tag]['cash_acc']=$trans->from_account_name;
                    $trans_records[$trans->tag]['cash_amount']=$trans->amount;

                    $trans_records[$trans->tag]['client_id']=is_null($trans->to_account_id)?'':$trans->to_account_id;
                    $trans_records[$trans->tag]['cash_id']=is_null($trans->from_account_id)?'':$trans->from_account_id;
                    break;
                case 2: 
                    $trans_records[$trans->tag]['bank_acc']=$trans->to_account_name;                    
                    $trans_records[$trans->tag]['swipe_amount']=$trans->amount;

                    $trans_records[$trans->tag]['bank_id']=is_null($trans->to_account_id)?'':$trans->to_account_id;
                    break;
                case 4:
                    if($trans->to_account_id==AGENT_COMMISSION_ACCOUNT_ID){
                        $trans_records[$trans->tag]['agent_acc']=$trans->from_account_name;
                        $trans_records[$trans->tag]['agent_commission']=$trans->amount;
                        
                        $trans_records[$trans->tag]['agent_id']=is_null($trans->from_account_id)?'':$trans->from_account_id;
                    } else if($trans->to_account_id==BANK_COMMISSION_ACCOUNT_ID){
                        $trans_records[$trans->tag]['bank_commission']=$trans->amount;
                    }
                    break;
            }
        }
        // var_dump($trans_records);
        // return;

        //FILTER FROM-TO DATE
        $this->filterDate_SwipeList($trans_records,'transaction_date','from_date', true);
        $this->filterDate_SwipeList($trans_records,'transaction_date','to_date', false);
        
        //FILTER BANK, AGENT, CLIENT AND CASH ID
        $this->filterData_SwipeList($trans_records,'bank_id','bank_id');
        $this->filterData_SwipeList($trans_records,'agent_id','agent_id');
        $this->filterData_SwipeList($trans_records,'client_id','client_id');
        $this->filterData_SwipeList($trans_records,'cash_id','cash_id');
        
        //FILTER MACHINE
        $this->filterData_SwipeList($trans_records,'machine_id','machine_id');

        
        // foreach ($trans_records as $key=>$transaction) {
        //     echo strtotime($transaction['transaction_date'] . ' 00:00') ."<br>";
        // }
        

        $isEdit = $this->applib->have_access_role(MODULE_RECEIPT_ID, "edit");
        $isDelete = $this->applib->have_access_role(MODULE_RECEIPT_ID, "delete");
        $isEdit .= $this->applib->have_access_role(MODULE_BANK_TRANSACTION, "edit");
        $isDelete .= $this->applib->have_access_role(MODULE_BANK_TRANSACTION, "delete");
        ...
        foreach ($trans_records as $key=>$transaction) {
            ...
            $action = '';
            if($isEdit) {
                $action .= '<a href="' . base_url("transaction/banktrans/" . $transaction['transaction_id']) . '" class="edit_button btn-primary btn-xs"><i class="fa fa-pen"></i></a>';    
                

            }
            if($isDelete) {
                $action .= '&nbsp; <a href="javascript:void(0);" class="delete_transaction btn-danger btn-xs" data-href="' . base_url('transaction/delete_receipt_transaction/' . $transaction['transaction_id']) . '"><i class="fa fa-trash"></i></a>';
            }
            ...
            $row[] = $action;
            $row[] = (!empty(strtotime($transaction['transaction_date']))) ? date('d-m-Y', strtotime($transaction['transaction_date'])) : '';
            $row[] = (isset($transaction['client']))?$transaction['client']:'';
            $row[] = (isset($transaction['cash_acc']))?$transaction['cash_acc']:'';;
            $row[] = (isset($transaction['cash_amount']))?$transaction['cash_amount']:'';;
            $row[] = (isset($transaction['machine']))?$transaction['machine']:'';;
            $row[] = (isset($transaction['bank_acc']))?$transaction['bank_acc']:'';;
            $row[] = (isset($transaction['swipe_amount']))?$transaction['swipe_amount']:'';;
            $row[] = (isset($transaction['agent_acc']))?$transaction['agent_acc']:'';;
            $row[] = (isset($transaction['agent_commission']))?$transaction['agent_commission']:'';; 
            $row[] = $transaction['agent_commission']  + $transaction['cash_amount'];;           
            $data[] = $row;
        }

        $output = array(
            "draw" => isset($_POST['draw'])?$_POST['draw']:0,
            "recordsTotal" => sizeof($data), //$this->datatable->count_all(), //, //->filtered() 
            "recordsFiltered" => 0, //$this->datatable->count_filtered(),
            "data" => $data,
        );
        echo json_encode($output);
    ...
    function banktransaction_datatable_group() {
        $query = "SELECT t.*,
                IFNULL((SELECT account_name FROM account WHERE account.account_id=t.from_account_id),'') from_account_name,
                IFNULL((SELECT account_name FROM account WHERE account.account_id=t.to_account_id),'') to_account_name,
                IFNULL((SELECT account_mobile_numbers FROM account WHERE account.account_id=t.to_account_id),'') account_mobile_numbers,
                sm.machine_name swipe_machine_name, sm.bank_acc_id
                FROM `transaction_entry` t
                INNER JOIN swipe_machines sm ON sm.machine_id=t.mach_id
                WHERE t.tag in (
                    SELECT tag FROM `transaction_entry` WHERE CHAR_LENGTH(tag) = 13
                    GROUP BY tag HAVING COUNT(transaction_id)>=2)
                ORDER by t.transaction_date DESC, t.tag";
        ...
        foreach($list as $trans){
            if(!isset($trans_records[$trans->tag])){
                ...
                $trans_records[$trans->tag]['machine']=$trans->swipe_machine_name;
                $trans_records[$trans->tag]['machine_id']=$trans->mach_id;
                $trans_records[$trans->tag]['transaction_note']=$trans->note;
            ...
            switch($trans->transaction_type){
                case 1:
                    $trans_records[$trans->tag]['client']=$trans->to_account_name;
                    $trans_records[$trans->tag]['account_mobile_numbers']=$trans->account_mobile_numbers;
                    $trans_records[$trans->tag]['cash_acc']=$trans->from_account_name;
                    $trans_records[$trans->tag]['cash_amount']=$trans->amount;
                    $trans_records[$trans->tag]['client_perc']=$trans->client_perc;
                ...
                case 4:
                    if($trans->to_account_id==AGENT_COMMISSION_ACCOUNT_ID){
                        $trans_records[$trans->tag]['agent_acc']=$trans->from_account_name;
                        $trans_records[$trans->tag]['agent_commission']=$trans->amount;
                        $trans_records[$trans->tag]['agent_percentage']=$trans->agent_perc;

                        $trans_records[$trans->tag]['agent_id']=is_null($trans->from_account_id)?'':$trans->from_account_id;
                    } else if($trans->to_account_id==BANK_COMMISSION_ACCOUNT_ID){
                        $trans_records[$trans->tag]['bank_commission']=$trans->amount;
                        $trans_records[$trans->tag]['bank_percentage']=$trans->bank_perc;
                        $trans_records[$trans->tag]['transaction_id_last']=$trans->transaction_id;
                    }
                    break;
            }
        }
...
//                 die();
        ...
        foreach ($trans_records as $transaction){
            ...
            if (!isset($sum[$transaction['bank_id']])) {
                // INITIATE ARRAYS.. THE PRODUCT ID IS USED AS ARRAY KEY
                $sum[$transaction['bank_id']]['bank_acc'] = (isset($transaction['bank_acc']))? $transaction['bank_acc'] :'';
                $sum[$transaction['bank_id']]['cash_amount'] = (isset($transaction['cash_amount']) && isset($transaction['client_perc']) ) ? ($transaction['cash_amount'] * $transaction['client_perc'] / 100) + $transaction['cash_amount']:'';
                $sum[$transaction['bank_id']]['net_in_bank'] = (isset($transaction['swipe_amount']) && isset($transaction['bank_percentage']) ) ? sprintf("%01.2f",$transaction['swipe_amount'] - ($transaction['swipe_amount'] * $transaction['bank_percentage'] / 100)):'';
                $sum[$transaction['bank_id']]['swipe_amount'] = (isset($transaction['swipe_amount']))?$transaction['swipe_amount']:'';
        ...
        foreach ($sum as $key=>$transaction) {
            $row = array();

            $row[] = $transaction['bank_acc'];
            $row[] = sprintf("%01.2f",$transaction['cash_amount']);
            $row[] = sprintf("%01.2f",$transaction['net_in_bank']);
            $row[] = $transaction['swipe_amount'];

//            $row[] = (isset($transaction['cash_amount']) && isset($transaction['client_perc']) ) ? ($transaction['cash_amount'] * $transaction['client_perc'] / 100) + $transaction['cash_amount']:'';
//
//            $row[] = (isset($transaction['swipe_amount']) && isset($transaction['bank_percentage']) ) ? sprintf("%01.2f",$transaction['swipe_amount'] - ($transaction['swipe_amount'] * $transaction['bank_percentage'] / 100)):'';
//            $row[] = (isset($transaction['swipe_amount']))?$transaction['swipe_amount']:'';
            $data[] = $row;
        }
    ...
    private function filterData_SwipeList(&$data,$data_key, $post_key){
        if(isset($_POST[$post_key]) && strlen($_POST[$post_key])>0 && ((int)$_POST[$post_key]>0)){
            foreach($data as $key=>$trans_record){
                if($trans_record[$data_key]!=$_POST[$post_key]){
                    unset($data[$key]);
                }
            }
        }
    } 
    private function filterDate_SwipeList(&$data,$data_key, $post_key,$greater_and_equal){        
        if(isset($_POST[$post_key]) && strlen($_POST[$post_key])>0){
            foreach($data as $key=>$trans_record){
                if($greater_and_equal){
                    if(strtotime($_POST[$post_key] . ' 00:00')>=strtotime($trans_record[$data_key] . ' 23:59')){
                        unset($data[$key]);
                    }
                } else {
                    $start = new DateTime($_POST[$post_key]);
                    $end = new DateTime($trans_record[$data_key]);
                    if(strtotime($_POST[$post_key] . ' 23:59')<=strtotime($trans_record[$data_key] . ' 00:00')){
                        unset($data[$key]);
                    }
                }
            }
    ...
    function delete_receipt_transaction($id = '') {
        ...
        $tag = $this->crud->get_column_value_by_id('transaction_entry','tag',array('transaction_id' => $id));
        if(!empty($tag) && isset($tag)) {
            $this->crud->delete('transaction_entry', array('tag' => $tag));
...
//                echo "<pre>"; print_r($invoice_data); exit;
    ...
    function get_machine_bank_data(){
        //Kajal 25/10/2022
        $response = ["status"=>NULL, "data"=>[]];
        //If mach_id has been posted
        if(isset($_POST['mach_id'])){
            $machine_id = (int)$_POST['mach_id'];
            //If mach_id is a valid number and bigger than Zero
            if($machine_id>0){
                //Execute fetch query
                $result = $this->crud->getFromSQL("SELECT bank_acc_id, bank_commission FROM swipe_machines WHERE machine_id=$machine_id;");
                $response['status'] = "OK";
    ...
    function get_curr_data(){
        $response = ["status"=>NULL, "data"=>[]];
        //If curr_id has been posted
        if(isset($_POST['acc_id'])){
            $account_id = (int)$_POST['acc_id'];

            if($account_id > 0){

                //Execute fetch query
                $result = $this->crud->getFromSQL("SELECT currency_id FROM account WHERE account_id='".$account_id."';");
                $response['status'] = "OK";
                $response['data'] = (!empty($result))?$result:[];
...
//        echo "<pre>"; print_r($get_dis_data); exit;
    ...
    function listbanktrans(){
        if($this->applib->have_access_role(MODULE_BANK_TRANSACTION,"view")) {
            $data = array();
            $data['transaction_type'] = 4;
            set_page('transaction/banktrans_list', $data);
        } else {
            $this->session->set_flashdata('success', false);
            $this->session->set_flashdata('message', 'You have not permission to access this page.');
            redirect('/');
    ...
    function listgroupbanktrans(){
        // if($this->applib->have_access_role(MODULE_RECEIPT_ID,"view")) {
        $data = array();
        $data['transaction_type'] = 4;
        set_page('transaction/banktrans_list_group', $data);
        // } else {
        //     $this->session->set_flashdata('success', false);
        //     $this->session->set_flashdata('message', 'You have not permission to access this page.');
        //     redirect('/');
    ...
    function banktrans($transaction_id = '') {
        $data = array();
        $transaction_date = $this->crud->get_column_value_by_id('company_settings','setting_value',array('company_id' => $this->logged_in_id,'setting_key' => 'payment_date'));
        if(!empty($transaction_date) && strtotime($transaction_date) > 0) {
            $data['transaction_date'] = date('d-m-Y',strtotime($transaction_date));
        ...
        if (isset($transaction_id) && !empty($transaction_id)) {
            ...
            if($this->applib->have_access_role(MODULE_BANK_TRANSACTION,"edit")) {
                $query = "SELECT t.*,
                IFNULL((SELECT account_name FROM account WHERE account.account_id=t.from_account_id),'') from_account_name,
                IFNULL((SELECT account_name FROM account WHERE account.account_id=t.to_account_id),'') to_account_name,
                IFNULL((SELECT account_mobile_numbers FROM account WHERE account.account_id=t.to_account_id),'') account_mobile_numbers,
                ...
                FROM `transaction_entry` t
                INNER JOIN swipe_machines sm ON sm.machine_id=t.mach_id
                WHERE t.tag in ('$transaction_data->tag')
                ORDER by t.transaction_date DESC, t.tag";
...
//                 echo "<pre>";
//                    print_r($trans_records[$transaction_data->tag]);
//                 echo "</pre>";
//                 die();
                $data['transaction_data'] = $trans_records[$transaction_data->tag];

                set_page('transaction/banktrans', $data);
            } else {
                $this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission to access this page.');
                redirect('/');
            ...
            if($this->applib->have_access_role(MODULE_BANK_TRANSACTION,"add")) {
                set_page('transaction/banktrans', $data);
            } else {
                $this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission 123 to access this page.');
                redirect('/');
            }            
        }
    }

    function save_banktransaction() {
        $post_data = $this->input->post();
        if (isset($post_data['tag']) && !empty($post_data['tag'])) {
            $this->crud->delete('transaction_entry', array('tag' => $post_data['tag']));
        }

            $transaction = [];
            
            // COMMON VALUE
            $transaction['mach_id'] = $post_data['drpMachine'];
            $transaction['bank_perc'] = $post_data['txtBankCharges'];
            $transaction['client_perc'] = $post_data['txtClientPercentage'];
            $transaction['agent_perc'] = $post_data['txtAgentCommission'];
            $transaction['note'] = $post_data['txtNotes'];
            $transaction['site_id'] = 0; //$post_data['site_id'];
            $transaction['tag'] = strtoupper(uniqid()); //MUST REMAIN SAME, IN NEXT ENTRIES.
            
            $transaction['transaction_date'] = (isset($post_data['txtDate']) && !empty($post_data['txtDate'])) ? date('Y-m-d', strtotime($post_data['txtDate'])) : null;
            $transaction['created_by'] = $this->logged_in_id;
            $transaction['user_created_by'] = $this->session->userdata()['login_user_id'];
            $client_id = $post_data['account_id'];
            if($client_id == -1){
                //NEW CLIENT
                $new_client = array(
                    'account_name' => $post_data['account_name'],
                    'account_mobile_numbers' => $post_data['txtMobile'],
                    'account_group_id' => SWIPE_ACCOUNT_GROUP_ID, //SUNDRY CREDITORS (OTHERS)
                    'created_by'=>$transaction['created_by'],
                    'created_at'=>$transaction['transaction_date'],
                    'user_created_by' => $transaction['user_created_by']
                );

                $result = $this->crud->insert('account', $new_client);
                $client_id = $this->db->insert_id();
            }
            /*ENTRY #1 : CLIENT/CASH*/
            $transaction['created_at'] = $this->now_time;
            $transaction['from_account_id'] = $post_data['drpCash'];
            $transaction['to_account_id'] = $client_id;
            $transaction['transaction_type'] = 1; //PAYMENT
            $transaction['currency_id'] = 4; 
            $transaction['base_currency_id'] = 4; 
            $transaction['base_currency_amount'] = $post_data['txtActualPaid']; 
            $transaction['amount'] = $post_data['txtActualPaid'];
            $result = $this->crud->insert('transaction_entry', $transaction);

            /*ENTRY #2: BANK/SWIPE*/
            $transaction['created_at'] = $this->now_time;
            $transaction['from_account_id'] = $client_id;
            $transaction['to_account_id'] = $post_data['drpBank'];
            $transaction['transaction_type'] = 2; //RECEIPT
            $transaction['currency_id'] = 4; 
            $transaction['base_currency_id'] = 4; 
            $transaction['base_currency_amount'] = $post_data['txtActualSwipe']; 
            $transaction['amount'] = $post_data['txtActualSwipe'];
            $result = $this->crud->insert('transaction_entry', $transaction);

            /*ENTRY #3: AGENT & AGENT-COMMISSION*/
            if(strlen($post_data['drpAgent'])>0 && $post_data['drpAgent']!=-1){
                $transaction['created_at'] = $this->now_time;
                $transaction['from_account_id'] = $post_data['drpAgent'];
                $transaction['to_account_id'] = AGENT_COMMISSION_ACCOUNT_ID; //AGENT-COMMISSION ACCOUNT
                $transaction['transaction_type'] = 4; //JOURNAL
                $transaction['currency_id'] = 4; 
                $transaction['base_currency_id'] = 4; 
                $transaction['base_currency_amount'] = $post_data['txtAgentAmount']; 
                $transaction['amount'] = $post_data['txtAgentAmount'];
                $result = $this->crud->insert('transaction_entry', $transaction);            
            }
            
            /*ENTRY #4: BANK & BANK-COMMISSION*/
            $transaction['created_at'] = $this->now_time;
            $transaction['from_account_id'] = $post_data['drpBank'];
            $transaction['to_account_id'] = BANK_COMMISSION_ACCOUNT_ID; //BANK-COMMISSION ACCOUNT
            $transaction['transaction_type'] = 4; //JOURNAL
            $transaction['currency_id'] = 4; 
            $transaction['base_currency_id'] = 4; 
            $transaction['base_currency_amount'] = round(((float)$post_data['txtActualSwipe'])*((float)$post_data['txtBankCharges'])/100,2); 
            $transaction['amount'] =round(((float)$post_data['txtActualSwipe'])*((float)$post_data['txtBankCharges'])/100,2);
            $result = $this->crud->insert('transaction_entry', $transaction);
            ...
            // UPLOAD FILES, IF ANY
                if (!empty($_FILES['vimgs']['name'][0])) {
                    $path = FCPATH . ACCOUNT_IMAGES . $client_id;
                    if(!file_exists($path)){
                        mkdir($path, 0777, true);
                    } else { $path.='/' ; }
                    if($this->upload_files($path, $transaction['tag'], $_FILES['vimgs'])===FALSE){
                        $this->session->set_flashdata('fail', true);
                        $this->session->set_flashdata('message', 'Failed to upload');
                    }
                }

        redirect('/transaction/banktrans');
        exit;
    }
