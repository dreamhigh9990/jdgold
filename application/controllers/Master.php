...
class Master extends CI_Controller
{
	...
	function currency(){
		if(!empty($_POST['id']) && isset($_POST['id'])){
			if($this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY,"view")) {
            	$result = $this->crud->get_data_row_by_id('currency_master ','id',$_POST['id']);
				$data = array(
					'id' => $result->id,
	...
	function currency_rate(){
		if(!empty($_POST['id']) && isset($_POST['id'])){
			if($this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY_RATE,"edit")) {
            	$result = $this->crud->get_data_row_by_id('currency_rate ','id',$_POST['id']);
				...
				set_page('master/currency_rate', $data);
	        } else {
	        	$this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission to access this page.');
	        	redirect('/');
			...
			if($this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY_RATE,"view") || $this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY_RATE,"add")) {
            	$data = array();
	        	set_page('master/currency_rate', $data);
	        } else {
	        	$this->session->set_flashdata('success', false);
	...
	function currency_rate_datatable()
	{
		
		$config['table'] = 'currency_rate a';
		$config['select'] = 'a.*, cm.currency_name, cm_multiply.currency_name as currency_multiplied_name';
		$config['column_order'] = array(null, 'a.id');
		$config['joins'][] = array(
			'join_table' => 'currency_master cm', 
			'join_by' => 'cm.id = a.currency_id', 
			'join_type' => 'left'
		);
		$config['joins'][] = array(
			'join_table' => 'currency_master cm_multiply', 
			'join_by' => 'cm_multiply.id = a.currency_multiplied_id', 
			'join_type' => 'left'
		);
		
		// $config['column_search'] = array('a.currency_name');
		$config['order'] = array('a.currency_name' => 'asc');
		...
		foreach ($list as $account_group) {
			...
			$action = '';
			// if($isEdit) {
			// 	$action .= '<form id="edit_' . $account_group->id . '" method="post" action="' . base_url() . 'master/currency_rate" class="pull-left">
			// 		<input type="hidden" name="id" id="id" value="' . $account_group->id . '">
			// 		<a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $account_group->id . '\').submit();" title="Edit Currency"><i class="fa fa-edit"></i></a>
			// 		</form>';
			// }
			// if($isDelete) {
				
			// 	$action .= ' &nbsp; <a href="javascript:void(0);" class="delete_button btn-danger btn-xs" data-href="' . base_url('master/delete_currency/' . $account_group->id) . '"><i class="fa fa-trash"></i></a>';
				
			// }
			
			$row[] = $action;
			// 			//Start : $row[] = $account_group->currency_id;

			// 			$query = "SELECT currency_rate.currency_id,  currency_master.currency_name
			// 			FROM currency_rate
			// 			INNER JOIN currency_master
			// 			ON currency_rate.currency_id=currency_master.id;";

			// $sql = $this->db->query($query);
			// return $query->array();
			// $query2 = "select * from currency_rate where currency_multiplied_id = '.$account_group->currency_multiplied_id.'";
			// $query3 = "select * from currency_master where currency_name = '.$account_group->currency_name.'";
			//take currency_name from here and give for 2nd column
			$row[] =  $account_group->currency_name;
		
			//End : $row[] = $account_group->currency_id;
			$row[] = "<input type='hidden' name='id' class='currency_rate_id' value='" . $account_group->id . "'><input type='text' class='currency_rate' name='currency_rate' value='".$account_group->multiplier."'>";
			$row[] =  $account_group->currency_multiplied_name;
			$data[] = $row;

			// $row[] = $action;
			// $row[] = $account_group->currency_name;
			// $row[] = "<input type='text' value='".$account_group->multiplier."'>";
			// $row[] = $account_group->currency_name;
			// $data[] = $row;
	...
	function save_currency_rate(){
		...
		if(isset($_POST['currency_rate']) && !empty($_POST['currency_rate'])){
			$currency_rate = $_POST['currency_rate'];
		} 

		if(isset($_POST['currency_rate_id']) && !empty($_POST['currency_rate_id'])){

			$data['multiplier'] = $_POST['currency_rate'];
			$where_array['id'] = $_POST['currency_rate_id'];
			$result = $this->crud->update('currency_rate', $data, $where_array);
	...
	function save_currency(){
		...
		if(isset($post_data['currency_name']) && !empty($post_data['currency_name'])){
		...
		if(isset($post_data['id']) && !empty($post_data['id'])){
			$id_result = $this->crud->get_same_by_val('currency_master','id','currency_name',trim($currency_name),'1','1');
			...
			// 	exit;
			// }
			$data['currency_name'] = trim($post_data['currency_name']);
			$data['is_default'] = $post_data['is_default'];
			$data['updated_at'] = $this->now_time;
			$data['updated_by'] = $this->logged_in_id;
			$data['user_updated_by'] = $this->session->userdata()['login_user_id'];
			$where_array['id'] = $post_data['id'];
			$result = $this->crud->update('currency_master', $data, $where_array);
			if ($result) {
				$return['success'] = "Updated";
			}
		} else {
			...
			$data['created_by'] = $this->logged_in_id;
			$data['user_created_by'] = $this->session->userdata()['login_user_id'];
			$result = $this->crud->insert('currency_master',$data);
			if($result){
				$return['success'] = "Added";
			}
		}
	...
	function currency_datatable()
	{
		...
		$config['table'] = 'currency_master a';
		$config['select'] = 'a.*';
		$config['column_order'] = array(null, 'a.currency_name');
		$config['column_search'] = array('a.currency_name');
		$config['order'] = array('a.currency_name' => 'asc');
		...
		$data = array();
		$isEdit = $this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY, "edit");
		$isDelete = $this->applib->have_access_role(MASTER_ACCOUNT_CURRENCY, "delete");
		foreach ($list as $account_group) {
			$row = array();
			$action = '';
			if($isEdit) {
				$action .= '<form id="edit_' . $account_group->id . '" method="post" action="' . base_url() . 'master/currency" class="pull-left">
					<input type="hidden" name="id" id="id" value="' . $account_group->id . '">
					<a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $account_group->id . '\').submit();" title="Edit Currency"><i class="fa fa-edit"></i></a>
					</form>';
			}
			...
			$row[] = $action;
			$row[] = $account_group->currency_name;
			$row[] = ($account_group->is_default == 'y' ? 'Yes' : 'No');
			$data[] = $row;
		}
		$output = array(
			"draw" => $_POST['draw'],
	...
	{
		$return = array();
        $result = $this->crud->delete('currency_master', array('id' => $id));
...
//                    echo "<pre>"; print_r($allDataInSheet); exit;
						...
						if (!empty($allDataInSheet)) {
							foreach ($allDataInSheet as $sheet_entry) 
							{
								...
								if(!empty($sheet_entry['A']))
								{
									...
									if(!empty($sheet_entry['D']))
									{
										$date_need_format     = str_replace('/', '-', substr($sheet_entry['D'], strpos($sheet_entry['D'], 'Date') + 7));
										
										

										$current_journal_date = date('Y-m-d', strtotime($sheet_entry['D']));
									...
									if (!empty($sheet_entry['B'])) {
										$account_name_data = trim($sheet_entry['B'], " ");

										

										$account_row = $this->crud->get_data_row_by_where('account', array('account_name' => $account_name_data));
										if (!empty($account_row)) {
											$account_id = $account_row->account_id;
										}
										else
										{
											$account_data['account_name'] = $account_name_data;
											$account_data['account_group_id'] = Group_C_Clients;
											$account_data['opening_balance'] = 0;
											$account_data['credit_debit'] = "";
											$account_data['account_city'] = 2;
											$account_data['account_postal_code'] = "";
											$account_data['account_state'] = 7;
											$account_data['account_pan'] = "";
									...
									if (!empty($sheet_entry['E'])) {
										$note=$sheet_entry['E'];
									}

									$post_data['transaction_date'] = $current_journal_date;
									$post_data['transaction_type']=$transaction_type;
									$post_data['account_id']=$account_id;
									$post_data['amount']=$transaction_amount;
									if ($amount >= 0 )
									{
										$post_data['from_account_id']=$account_id;
										
										$post_data['to_account_id'] = Account_C;
									}
									else 
									{
										$post_data['to_account_id'] = $account_id;
										$post_data['from_account_id'] = Account_C;
									}
									
								
									$post_data['site_id']=0;
									$post_data['receipt_no']='';
									$post_data['note']=$note;
									...
									$transaction_entry = $this->crud->insert('transaction_entry',$post_data);

								}// end if if(!empty($sheet_entry['A']))

							}

							$data['import_group_c_data'] = $import_group_c_data;
				...
				if($radio_type==15)
				{
                    ...
                    $objPHPExcel = PHPExcel_IOFactory::load($_FILES['userfile']['tmp_name'],'Excel5');
                    $allDataInSheet = $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
					
					unset($allDataInSheet[1]);
                    $import_deposit = array();
                    ...
                    foreach ($allDataInSheet as $key => $excel_row) {
						...
						$name = explode(" - ",$excel_row['B']);
						$datetime = explode(" ",$excel_row['C']);

						$this->db->select(" account_id ");
						$this->db->from("account");
						$this->db->where("account_name",$name[0]);
						$query = $this->db->get();
						$row = $query->row();
						if($row->account_id){
							$accountno_account_id = $row->account_id;
						}else{
							$data = array(
								'account_name' => $name[0],
								'remote_account_name' => $name[1],
								'account_group_id' => "63",
								'created_at' => $this->now_time,
							...
							$accountno_account_id = $this->db->insert_id();
						}


						
						$this->db->select(" ib_account_id ");
						$this->db->from("transaction_entry");
						$this->db->where("accountno_account_id",$accountno_account_id);
						$this->db->where('ib_account_id is NOT NULL', NULL, FALSE);
						$sql_accountno_account_id = $this->db->get();
						$row_acno_account_id = $sql_accountno_account_id->row();
						$ib_account_id = 0;

						if($row_acno_account_id->ib_account_id){
							$ib_account_id = $row_acno_account_id->ib_account_id;
						}

						$from_account_id   = 458; //458 is "Erp Auto Entry" account
						$to_account_id     = ($ib_account_id == 0)?$accountno_account_id:$ib_account_id;
						$transaction_type  = 1; 


						if($ib_account_id > 0){
							$account_id_to_store = $ib_account_id;
						} else {
							$account_id_to_store = $accountno_account_id;
						}

						$this->db->select(" transaction_id ");
						$this->db->from("transaction_entry");
						$this->db->where("transaction_date",$datetime[0]);
						$this->db->where("trans_time",$datetime[1]);
						$sql_transaction_id = $this->db->get();
						$transaction_id = $sql_transaction_id->row();
						if(!$transaction_id->transaction_id){
							$data = array(
								'ib_account_id' => $account_id_to_store,
								'accountno_account_id' => $accountno_account_id,
								'transaction_date' => $datetime[0],
								'trans_time' =>  $datetime[1],
								'account_id' => $account_id_to_store,
								'from_account_id' => $from_account_id,
								'to_account_id' => $to_account_id,
								'transaction_type' => 1,
								'amount' => $excel_row['F'],
								'remote_note' => json_encode($excel_row),
								'remote_accountno' => trim($name[0]),
								'remote_type' => $excel_row['A'],
								'remote_currency' => $excel_row['E'],
								'created_at' => date("Y-m-d H:i:s"),
								'created_by' => 2,
								'updated_at' => date("Y-m-d H:i:s")
								);
							$this->db->insert('transaction_entry',$data);
							$accountno_account_id = $this->db->insert_id();
				...
				if($radio_type==16)
				{
                    ...
                    foreach ($allDataInSheet as $key => $excel_row) {
						...
						$from_account_id   = ($ib_account_id == 0)?$accountno_account_id:$ib_account_id;
						$to_account_id     = 458; //458 is "Erp Auto Entry" account
						$transaction_type  = 2; 
    ...
    function user_user_rights(){
    	if($this->applib->have_access_role(MASTER_USER_ID,"view")) {
			...
			$data['accounts'] = $this->crud->getFromSQL("SELECT * FROM `account`  ORDER BY `account_name` ASC ");


			$user_default_cash_acc_id=$this->crud->get_column_value_by_id("user","user_default_cash_acc_id",array('user_id' => $user_id));

			$set_allowed_account_ids=$this->crud->get_column_value_by_id("user_allow_accounts","allowed_account_id",array('user_id' => $user_id));
			$set_allowed_user_ids=$this->crud->get_column_value_by_id("user_allow_user_view","user_allow_user_id",array('user_id' => $user_id));

			$data['user_default_cash_acc_id']=$user_default_cash_acc_id;
	        $data['modules_roles'] = $this->app_model->getModuleRoles();
	        $data['user_roles'] = $this->app_model->getUserRoleIDS($user_id);
	        $data['set_allowed_account_ids'] = $this->app_model->getAllowAccountIDS($user_id);
	        $data['set_allowed_account_ids_view'] = $this->app_model->getAllowAccountIDSView($user_id);
	        $data['set_allowed_user_ids_view'] = $this->app_model->getAllowUserIDS($user_id);

			// echo "<pre>";
			// print_r($data['set_allowed_account_ids_view']);
			// echo "</pre>";
			// exit;
    ...
    function update_roles(){
        ...
        $allowed_accounts_view = $this->input->post("allowed_accounts_view");
        $allowed_users_view = $this->input->post("allowed_users_view");

        $user_default_cash_acc_id = $this->input->post("user_default_cash_acc_id");
		
        if(intval($user_id) > 0){
            $roles = $this->input->post("roles");

			$sql_update = "update user set user_default_cash_acc_id=".$user_default_cash_acc_id." WHERE user_id='$user_id'";
//			 echo $sql_update;
//			exit;
            $this->crud->execuetSQL($sql_update);
			...
			$sql_user_allow = "DELETE FROM user_allow_accounts WHERE user_id='$user_id'";
            $this->crud->execuetSQL($sql_user_allow);
			
			$sql_user_allow_view = "DELETE FROM user_allow_accounts_view WHERE user_id='$user_id'";
            $this->crud->execuetSQL($sql_user_allow_view);

			if($allowed_accounts)
			{
				$allowed_accounts_arr=array();
				foreach($allowed_accounts as $allowed_account)
				{
					$data = array(
						'user_id' => $user_id,
						'allowed_account_id' => $allowed_account,
					);
					array_push($allowed_accounts_arr, $data);

				}
				$this->db->insert_batch('user_allow_accounts', $allowed_accounts_arr);

			}
			if($allowed_accounts_view)
			{
				$allowed_accounts_view_arr=array();
				foreach($allowed_accounts_view as $allowed_account_view)
				{
					$data = array(
						'user_id' => $user_id,
						'allowed_account_id' => $allowed_account_view,
					);
					array_push($allowed_accounts_view_arr, $data);

				}
				$this->db->insert_batch('user_allow_accounts_view', $allowed_accounts_view_arr);

			}
			if($allowed_users_view)
			{
				$allowed_users_view_arr=array();
				foreach($allowed_users_view as $allowed_user_view)
				{
					$data = array(
						'user_id' => $user_id,
						'user_allow_user_id' => $allowed_user_view,
					);
					array_push($allowed_users_view_arr, $data);

				}
				$this->db->insert_batch('user_allow_user_view', $allowed_users_view_arr);
...
//			}
	...
	function bank_machine(){
		//Kajal - 20/10/2022 18:42
    ...
    //             $this->session->set_flashdata('message', 'You have not permission to access this page.');
	  ...
	  //       }
		...
		// } else {
			//change role/rights
			if($this->applib->have_access_role(MASTER_BANK_SWIPE_GROUP_ID,"view") || $this->applib->have_access_role(MASTER_BANK_SWIPE_GROUP_ID,"add")) {
					$data = array();
					set_page('master/bank_machine', $data);
	        } else {
	        	$this->session->set_flashdata('success', false);
                $this->session->set_flashdata('message', 'You have not permission to access this page.');
	        	redirect('/');
	...
	function bank_datatable(){
		$config['table'] = 'swipe_machines mach';
		$config['select'] = 'mach.machine_id, mach.bank_acc_id, mach.machine_name, mach.bank_commission, acc.account_name';
		$config['joins'][] = array('join_table' => 'account acc', 'join_by' => 'acc.account_id = mach.bank_acc_id', 'join_type' => 'left');
		// $config['order'] = array('acc.account_name' => 'asc');
		// $config['column_search'] = array('acc.account_name');
		$config['wheres'][] = array('column_name' => 'acc.account_group_id', 'column_value' => 21);
		...
		//NEED TO UPDATE THIS TO PROPER RIGHTS.
		// $isEdit = $this->applib->have_access_role(MASTER_CITY_ID, "edit");
		// $isDelete = $this->applib->have_access_role(MASTER_CITY_ID, "delete");
		$isEdit = TRUE;
		$isDelete = TRUE;
		...
		foreach ($list as $bank) {
			...
			$action = '';
			if($isEdit) {
				$action .= '<form id="edit_' . $bank->machine_id . '" method="post" action="' . base_url() . 'master/city" class="pull-left">
							<input type="hidden" name="id" id="id" value="' . $bank->machine_id . '">
							<a class="edit_button btn-primary btn-xs" href="javascript:{}" onclick="document.getElementById(\'edit_' . $bank->machine_id . '\').submit();" title="Edit City"><i class="fa fa-edit"></i></a>
						</form>';
			}
			if($isDelete) {
				$action .= ' &nbsp; <a href="javascript:void(0);" class="delete_button btn-danger btn-xs" data-href="' . base_url('master/delete/' . $bank->machine_id) . '"><i class="fa fa-trash"></i></a>';
			}
			
			$row[] = $action;
			$row[] = $bank->machine_name;
	...
	function save_bank_machine(){
		...
		if(isset($post_data['id']) && !empty($post_data['id'])){
			...
			$v_id = $this->crud->get_same_by_val('swipe_machines','machine_id','machine_name',trim($post_data['machine_name']),'bank_acc_id',$post_data['bank_id']);
			if(!empty($v_id)) {
				$return['error'] = "Exist";
				print json_encode($return);
				exit;
			}
			$data['bank_acc_id'] = $post_data['bank_id'];
			$data['machine_name'] = $post_data['machine_name'];
			$data['bank_commission'] = round($post_data['bank_commission'],2);
			$data['created_by'] = $this->logged_in_id;
			$result = $this->crud->insert('swipe_machines',$data);
			if($result){
				$return['success'] = "Added";
			}
		}
