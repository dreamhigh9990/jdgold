...
class Report extends CI_Controller
{
    ...
    function ledger($account_id = '')
    {
        if ($this->applib->have_access_role(MODULE_LEDGER_ID, "view")) {
            ...
            if(isset($_GET['display']) && $_GET['display'] == "image"){
                set_page('report/ledger_report_saveimg', $data);
            }else{
    ...
    function depowithdraw_datatable_new()
    {
        ...
        $update_ib_account_id = $_POST['update_ib_account_id'];
        ...
        if (($account_id == null || $account_id == 'null' || empty($account_id)) && (empty($sub_account_id))) {
            ...
            foreach ($ledger_data as $list_row) {
                    ...
                    if (in_array($check_acc_no, $allow_accounts)) {
                        ...
                        if ($set_multiplier == NULL || $set_multiplier == 0) {
                            $account_currency_id = $this->crud->get_val_by_id('account', $list_row->ib_account_id, 'account_id', 'currency_id');
                            
                            $set_multiplier_array = $this->db->select('multiplier')->where('currency_id',1)->where('currency_multiplied_id',$account_currency_id)->get('currency_rate')->row_array();
                            
                            $set_multiplier = $set_multiplier_array['multiplier'];
                            //AND currency_id = 1
                        }else if($update_ib_account_id == ''){
                            $account_currency_id = $this->crud->get_val_by_id('account', $list_row->ib_account_id, 'account_id', 'currency_id');
                        ...
                        $row[] = ''; //remotenote for other data column
                        $row[] = $ib_account_name;
...
//                $row[] = '<select name="row_account_id[]" class="row_account_id account_id"></select><input type="hidden" class="row_transaction_id" value="' . $list_row->transaction_id . '"><input type="hidden" class="row_set_transaction_id" value="' . $set_lb_account_id . '">';
    ...
    function ledger_datatable_new()
    {
        ...
        $to_date = date('Y-m-d', strtotime($_POST['daterange_2']));
        $account_id = $_POST['account_id'];
        $open_deposit = 0; $open_withdraw = 0; $open_received = 0; $open_paid = 0;
        $total_deposit = 0; $total_withdraw = 0; $total_received = 0; $total_paid = 0;

        $account_row = $this->crud->get_data_row_by_where('account', array('account_id' => $account_id));
        $account_currency_id = $account_row->currency_id;
        if($account_row->credit_debit == 1)
          $account_opening_balance = $account_row->opening_balance;
        else
          $account_opening_balance = 0 - $account_row->opening_balance;
            
    //For Deposit : Start
        $deposit_sql = "SELECT * FROM `transaction_entry` WHERE `from_account_id` = 458 AND `to_account_id` = ".$account_id;
        $deposit_sql_arr = $this->crud->getFromSQL($deposit_sql);

	    foreach ($deposit_sql_arr as $deposit_row) {
			if($deposit_row->currency_id == $account_currency_id && $deposit_row->transaction_date < $from_date) {
			  $open_deposit += $deposit_row->amount;
			}
			else if($deposit_row->currency_id == $account_currency_id && $deposit_row->transaction_date >= $from_date && $deposit_row->transaction_date <= $to_date) {
			  $total_deposit += $deposit_row->amount;
			}
			else if($deposit_row->base_currency_id == $account_currency_id && $deposit_row->transaction_date < $from_date) {
                $open_deposit += $deposit_row->amount;
              }
            else if($deposit_row->base_currency_id == $account_currency_id && $deposit_row->transaction_date >= $from_date && $deposit_row->transaction_date <= $to_date) {
                $total_deposit += $deposit_row->base_currency_amount;
            }
          }
    //For Deposit : End
    //For Withdraw : Start
        $withdraw_sql = "SELECT * FROM `transaction_entry` WHERE `to_account_id` = 458 AND `from_account_id` = ".$account_id;
        $withdraw_sql_arr = $this->crud->getFromSQL($withdraw_sql);
	    
        foreach ($withdraw_sql_arr as $withdraw_row) {
			if($withdraw_row->currency_id == $account_currency_id && $withdraw_row->transaction_date < $from_date) {
			  $open_withdraw += $withdraw_row->amount;
			}
			else if($withdraw_row->currency_id == $account_currency_id && $withdraw_row->transaction_date >= $from_date && $withdraw_row->transaction_date <= $to_date) {
			  $total_withdraw += $withdraw_row->amount;
			}
			else if($withdraw_row->base_currency_id == $account_currency_id && $withdraw_row->transaction_date < $from_date) {
                $open_withdraw += $withdraw_row->amount;
              }
            else if($withdraw_row->base_currency_id == $account_currency_id && $withdraw_row->transaction_date >= $from_date && $withdraw_row->transaction_date <= $to_date) {
                $total_withdraw += $withdraw_row->base_currency_amount;
            }
        }    
    //For Withdraw : End
    //For Received : Start
        $received_sql = "SELECT * FROM `transaction_entry` WHERE `to_account_id` != 458 AND `from_account_id` = ".$account_id;
        $received_sql_arr = $this->crud->getFromSQL($received_sql);
	    
        foreach ($received_sql_arr as $received_row) {
			if($received_row->currency_id == $account_currency_id && $received_row->transaction_date < $from_date) {
			  $open_received += $received_row->amount;
			}
			else if($received_row->currency_id == $account_currency_id && $received_row->transaction_date >= $from_date && $received_row->transaction_date <= $to_date) {
			  $total_received += $received_row->amount;
			}
			else if($received_row->base_currency_id == $account_currency_id && $received_row->transaction_date < $from_date) {
			  $open_received += $received_row->amount;
			}
			else if($received_row->base_currency_id == $account_currency_id && $received_row->transaction_date >= $from_date && $received_row->transaction_date <= $to_date) {
			  $total_received += $received_row->base_currency_amount;
			}
        }
    //For Received : End
    //For Paid : Start
        $paid_sql = "SELECT * FROM `transaction_entry` WHERE `from_account_id` != 458 AND `to_account_id` = ".$account_id;
        $paid_sql_arr = $this->crud->getFromSQL($paid_sql);

        foreach ($paid_sql_arr as $paid_row) {
            if($paid_row->currency_id == $account_currency_id && $paid_row->transaction_date < $from_date) {
              $open_paid += $paid_row->amount;
            }
            else if($paid_row->currency_id == $account_currency_id && $paid_row->transaction_date >= $from_date && $paid_row->transaction_date <= $to_date) {
              $total_paid += $paid_row->amount;
            }
            else if($paid_row->base_currency_id == $account_currency_id && $paid_row->transaction_date < $from_date) {
                $open_paid += $paid_row->amount;
            }
            else if($paid_row->base_currency_id == $account_currency_id && $paid_row->transaction_date >= $from_date && $paid_row->transaction_date <= $to_date) {
                $total_paid += $paid_row->base_currency_amount;
            }
          }

    //For Paid : End

        $opening = $account_opening_balance - $open_deposit + $open_withdraw + $open_received - $open_paid;        
        ...
        $total[] = '<b style="font-weight: 600; font-size: 16px;">Opening</b>';
        $total_open[] =  '<b style="font-weight: 600; font-size: 16px;">Opening</b>';
        if($opening <= 0){
            $total_open[] = '<b class="red-text">'.number_format($opening,2,".","").'</b>';
        }else{
            $total_open[] = '<b class="blue-text">'.number_format($opening,2,".","").'</b>';
        ...
        $total_deposit_d[] =  '<b style="font-weight: 600; font-size: 16px;">Total Deposit</b>';
        $total_deposit_d[] = '<b class="blue-text">'.number_format($total_deposit,2,".","").'</b>';
        $total_deposit_d[] = '';
        $total_deposit_d[] = '';
        $total_deposit_d[] = '';
        $total_deposit_d[] = '';
        ...
        $total_deposit_d[] = '';
        $total_deposit_d[] = '';
        $data[] = $total_deposit_d;
        
        $tota2[] = '<b style="font-weight: 600; font-size: 16px;">Total Withdraw</b>';
        $tota2[] = '<b class="red-text">'.number_format($total_withdraw,2,".","").'</b>';
        $tota2[] = '';
        $tota2[] = '';
        $tota2[] = '';
        $tota2[] = '';
        ...
        $total3[] = '<b style="font-weight: 600; font-size: 16px;">Total Received</b>';
        $total3[] = '<b class="blue-text">'.number_format($total_received,2,".","").'</b>';
        $total3[] = '';
        $total3[] = '';
        $total3[] = '';
        $total3[] = '';
        ...
        $total4[] = '<b style="font-weight: 600; font-size: 16px;">Total Paid</b>';
        $total4[] = '<b class="red-text">'.number_format($total_paid,2,".","").'</b>';
        $total4[] = '';
        $total4[] = '';
        $total4[] = '';
        $total4[] = '';
        ...
        $total_closing[] = '<b style="font-weight: 600; font-size: 16px;">Closing</b>';
        if($closing <= 0){

            $total_closing[] = '<b class="red-text">'.number_format($closing,2,".","").'</b>';
        }else{
            $total_closing[] = '<b class="blue-text">'.number_format($closing,2,".","").'</b>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "data" => $data,
            "closing_amount" => number_format((float) $total, 2, '.', ''),
        );
    ...
    function ledger_datatable()
    {
        ...
        foreach ($ledger_data as $list_row) {
            ...
            $created_at = (!empty($list_row->created_at)) ? $list_row->created_at : '';
            $time_part = substr($created_at, 11, 8);
            $receipt_no = (!empty($list_row->receipt_no)) ? $list_row->receipt_no : '';
            ...
            if ($list_row->tran_type == "opening_balance") {
                $particular = "<b style='font-weight: 600; font-size: 16px;'>Opening Balance</b>";
                // print_r($list_row);
                if ($list_row->opening_amount < 0) {
                    $debit_amt = $list_row->opening_amount;
                    $opening_amount = $list_row->opening_amount;
                ...
                $particular = '<b style="font-weight: 600; font-size: 16px;">From Payment</b>';

                $isEdit = $this->applib->have_access_role(MODULE_PAYMENT_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_PAYMENT_ID, "delete");

                if ($isEdit) {
                    $particular = '<a href="' . base_url("transaction/payment/" . $list_row->transaction_id) . '"><b style="font-weight: 600; font-size: 16px;">Payment</b></a>';
                } else {
                    $particular = '<b style="font-weight: 600; font-size: 16px;">Payment</b>';
                }

                if ($isDelete) {
                    $delete_link = '<a href="javascript:void(0);" class="delete_button" data-href="' . base_url('transaction/delete_transaction/' . $list_row->transaction_id) . '"><i class="fa fa-trash" area-hidden="true" style="color:red">&nbsp;</i></a>';
                } else {
                    $delete_link = '';
                }
                ...
                if ($isEdit) {
                    $particular = '<a href="' . base_url("transaction/mbtrans/" . $list_row->transaction_id) . '"><b style="font-weight: 600; font-size: 16px;">Payment</b></a>';
                } else {
                    $particular = '<b style="font-weight: 600; font-size: 16px;">Payment</b>';
                }
                ...
                $particular = "<b style='font-weight: 600; font-size: 16px;'>From Receipt</b>";

                $isEdit = $this->applib->have_access_role(MODULE_RECEIPT_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_RECEIPT_ID, "delete");

                if ($isEdit) {
                    $particular = '<a href="' . base_url("transaction/mbtrans/" . $list_row->transaction_id) . '"><b style="font-weight: 600; font-size: 16px;">Receipt</b></a>';
                } else {
                    $particular = '<b style="font-weight: 600; font-size: 16px;">Receipt</b>';
                }
                ...
                $particular = '<b style="font-weight: 600; font-size: 16px;">To Receipt</b>';
                ...
                $bill_no = $list_row->contra_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">From Contra</b>';

                $isEdit = $this->applib->have_access_role(MODULE_CONTRA_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_CONTRA_ID, "delete");

                if ($isEdit) {
                    $particular = '<a href="' . base_url("contra/contra/" . $list_row->transaction_id) . '"><b style="font-weight: 600; font-size: 16px;">Contra</b></a>';
                } else {
                    $particular = '<b style="font-weight: 600; font-size: 16px;">Contra</b>';
                }
                ...
                $bill_no = $list_row->contra_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">To Contra</b>';
                ...
                $bill_no = $list_row->contra_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">From Journal</b>';
                ...
                if ($isEdit) {
                    if (!empty($list_row->journal_id)) {
                        $particular = '<a href="' . base_url("journal/journal_type2/" . $list_row->journal_id) . '"><b style="font-weight: 600; font-size: 16px;">Journal</b></a>';
                    } else {
                        $particular = '<a href="' . base_url("journal/journal/" . $list_row->transaction_id) . '"><b style="font-weight: 600; font-size: 16px;">Journal</b></a>';
                    }

                } else {
                    $particular = '<b style="font-weight: 600; font-size: 16px;">Journal</b>';
                }
                ...
                $bill_no = $list_row->contra_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">To Journal</b>';
                ...
                $bill_no = $list_row->bill_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Purchase Invoice</b>';
                ...
                if ($isEdit) {
                    if ($is_single_line_item == 1 || 1) {
                        if ($list_row->invoice_type == 3) {
                            $particular = '<form id="edit_' . $list_row->purchase_invoice_id . '" method="post" action="' . base_url('transaction/order_type2') . '" style="width: 25px; display: initial;" >
                                            <input type="hidden" name="purchase_invoice_id" id="purchase_invoice_id" value="' . $list_row->purchase_invoice_id . '">
                                            <a class="edit_button" href="javascript:{}" onclick="document.getElementById(\'edit_' . $list_row->purchase_invoice_id . '\').submit();" title="Edit Invoice"><b style="font-weight: 600; font-size: 16px;">Purchase Invoice</b></a>
                                        </form> ';
                        } else {
                            $particular = '<form id="edit_' . $list_row->purchase_invoice_id . '" method="post" action="' . base_url($list_row->invoice_type == '2' ? 'transaction/sales_purchase_transaction/purchase' : 'purchase/order') . '" style="width: 25px; display: initial;" >
                                            <input type="hidden" name="purchase_invoice_id" id="purchase_invoice_id" value="' . $list_row->purchase_invoice_id . '">
                ...
                $bill_no = $list_row->bill_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Against Purchase Invoice</b>';
                ...
                $bill_no = $list_row->credit_note_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Credit Note</b>';
                ...
                if ($isEdit) {
                    if ($is_single_line_item == 1 || 1) {
                        $particular = '<form id="edit_' . $list_row->credit_note_id . '" method="post" action="' . base_url() . 'transaction/sales_purchase_transaction/credit_note" style="width: 25px; display: initial;" >
                                <input type="hidden" name="credit_note_id" id="credit_note_id" value="' . $list_row->credit_note_id . '">
                                <a class="edit_button" href="javascript:{}" onclick="document.getElementById(\'edit_' . $list_row->credit_note_id . '\').submit();" title="Edit Credit Note"><b style="font-weight: 600; font-size: 16px;">Credit Note</b></a>
                            </form> ';
                    } else {
                        $particular = '<form id="edit_' . $list_row->credit_note_id . '" method="post" action="' . base_url() . 'credit_note/add" style="width: 25px; display: initial;" >
                                <input type="hidden" name="credit_note_id" id="credit_note_id" value="' . $list_row->credit_note_id . '">
                ...
                $bill_no = $list_row->credit_note_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Against Credit Note</b>';
                $isEdit = $this->applib->have_access_role(MODULE_CREDIT_NOTE_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_CREDIT_NOTE_ID, "delete");
                ...
                $bill_no = $this->applib->format_invoice_number($list_row->sales_invoice_id, $list_row->sales_invoice_date);
                $particular = '<b style="font-weight: 600; font-size: 16px;">Sales Invoice</b>';
                $isEdit = $this->applib->have_access_role(MODULE_SALES_INVOICE_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_SALES_INVOICE_ID, "delete");
                ...
                if ($isEdit) {
                    if ($is_single_line_item == 1 || 1) {
                        $particular = '<form id="edit_' . $list_row->sales_invoice_id . '" method="post" action="' . base_url() . 'transaction/sales_purchase_transaction/sales" style="width: 25px; display: initial;" >
                                <input type="hidden" name="sales_invoice_id" id="sales_invoice_id" value="' . $list_row->sales_invoice_id . '">
                                <a class="edit_button" href="javascript:{}" onclick="document.getElementById(\'edit_' . $list_row->sales_invoice_id . '\').submit();" title="Edit Invoice"><b style="font-weight: 600; font-size: 16px;">Sales Invoice</b></a>
                            </form> ';
                    } else {
                        $particular = '<form id="edit_' . $list_row->sales_invoice_id . '" method="post" action="' . base_url() . 'sales/invoice" style="width: 25px; display: initial;" >
                                <input type="hidden" name="sales_invoice_id" id="sales_invoice_id" value="' . $list_row->sales_invoice_id . '">
                ...
                $bill_no = $this->applib->format_invoice_number($list_row->sales_invoice_id, $list_row->sales_invoice_date);
                $particular = '<b style="font-weight: 600; font-size: 16px;">Against Sales Invoice</b>';
                $isEdit = $this->applib->have_access_role(MODULE_SALES_INVOICE_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_SALES_INVOICE_ID, "delete");
                ...
                $bill_no = $list_row->debit_note_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Debit Note</b>';
                $isEdit = $this->applib->have_access_role(MODULE_DEBIT_NOTE_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_DEBIT_NOTE_ID, "delete");
                ...
                if ($isEdit) {
                    if ($is_single_line_item == 1 || 1) {
                        $particular = '<form id="edit_' . $list_row->debit_note_id . '" method="post" action="' . base_url() . 'transaction/sales_purchase_transaction/debit_note" style="width: 25px; display: initial;" >
                                <input type="hidden" name="debit_note_id" id="debit_note_id" value="' . $list_row->debit_note_id . '">
                                <a class="edit_button" href="javascript:{}" onclick="document.getElementById(\'edit_' . $list_row->debit_note_id . '\').submit();" title="Edit Debit Note"><b style="font-weight: 600; font-size: 16px;">Debit Note</b></a>
                            </form> ';
                    } else {
                        $particular = '<form id="edit_' . $list_row->debit_note_id . '" method="post" action="' . base_url() . 'debit_note/add" style="width: 25px; display: initial;" >
                                <input type="hidden" name="debit_note_id" id="debit_note_id" value="' . $list_row->debit_note_id . '">
                ...
                $bill_no = $list_row->debit_note_no;
                $particular = '<b style="font-weight: 600; font-size: 16px;">Against Debit Note</b>';
                $isEdit = $this->applib->have_access_role(MODULE_DEBIT_NOTE_ID, "edit");
                $isDelete = $this->applib->have_access_role(MODULE_DEBIT_NOTE_ID, "delete");
            ...
            $row[] = '<b style="font-weight: 600; font-size: 16px;">'.$tr_date.'</b>';
            if(empty($tr_time)){
                
                $row[] = '<b style="font-weight: 600; font-size: 16px;">'.$time_part.'</b>';
            }else{

                $row[] = '<b style="font-weight: 600; font-size: 16px;">'.$tr_time.'</b>';
            }
            // $row[] = $remote_accountno;
            $row[] = '<b style="font-weight: 600; font-size: 16px;">'.substr($transaction_entry_note,0,9).'</b>';
            // $row[] = '<b style="font-weight: 600; font-size: 16px;">'.$bill_no.'</b>';
            if(!empty($remote_accountno) && !empty($receipt_no)){

                $row[] = '<b style="font-weight: 600; font-size: 16px;">'.substr($remote_accountno,0,5).' : '.substr($receipt_no,0,5).'</b>';
            }else if(empty($remote_accountno)){

                $row[] = '<b style="font-weight: 600; font-size: 16px;">'.substr($receipt_no,0,5).'</b>';
            }else if(empty($receipt_no)){

                $row[] = '<b style="font-weight: 600; font-size: 16px;">'.substr($remote_accountno,0,5).'</b>';
            }
            // $row[] = isset( $list_row->qty_total ) ? $list_row->qty_total : 0;
            // $row[] = isset( $list_row->vehicle_no ) ? $list_row->vehicle_no : '';
            // $row[] = isset( $unit_data[0]->unit_name ) ? $unit_data[0]->unit_name : '';
            $row[] = $particular;
            $row[] = '<b style="font-weight: 600; font-size: 16px;">'.substr((isset($list_row->opp_acc_name) ? $list_row->opp_acc_name : ''),0,9).'</b>';
            if($credit_amt <= 0){

                $row[] = '<b class="red-text">'.$credit_amt.'</b>';
            }else{
                $row[] = '<b class="blue-text">'.$credit_amt.'</b>';
            }
            if($debit_amt <= 0){

                $row[] = '<b class="red-text">'.$debit_amt.'</b>';
            }else{
                $row[] = '<b class="blue-text">'.$debit_amt.'</b>';
            }
            if($balance_amt <= 0){

                $row[] = '<b class="red-text">'.$balance_amt.'</b>';
            }else{
                $row[] = '<b class="blue-text">'.$balance_amt.'</b>';
        ...
        $total[] = '';
        $total[] = '';
        $total[] = '<b style="font-weight: 600; font-size: 16px;">Total</b>';
        if($total_credit_amt <= 0){

            $total[] = '<b class="red-text">'.$total_credit_amt.'</b>';
        }else{
            $total[] = '<b class="blue-text">'.$total_credit_amt.'</b>';

        }
        if($total_debit_amt <= 0){

            $total[] = '<b class="red-text">'.$total_debit_amt.'</b>';
        }else{
            $total[] = '<b class="blue-text">'.$total_debit_amt.'</b>';
        ...
        if($total <= 0){
            ...
            $total2[] = '<b class="red-text">'.$total.'</b>';
        }else{
            $total2[] = '<b class="blue-text">'.$total.'</b>';
    ...
    function ganpati()
    {
        set_page('dashboard-ganpati', $data);
    }
    ...
    function trial_balance_datatable_1()
    {
        $group_id = '';
        ...
        $all_allow_account_arr = $this->db->select('website_module_id')->where(['user_id' => $user_id,'website_module_id'=>MODULE_ALLOWED_ACCOUNTS_VIEW])->get('user_roles')->row_array();
        
        $all_allow_account = $all_allow_account_arr['website_module_id'];
        
        $allowed_account_ids_comma = array();
        if(empty($all_allow_account)){
            $allow_account_arr = $this->db->select('allowed_account_id')->where(['user_id' => $user_id])->get('user_allow_accounts_view')->result_array();
            foreach ($allow_account_arr as $value) {
                $allowed_account_ids = $value['allowed_account_id'];
                $allowed_account_id = array('a.account_id' => $allowed_account_ids);
                $allowed_account_ids_comma[] = $value['allowed_account_id'];
            }
            if(empty($allowed_account_ids_comma)){
                $allowed_account_ids_comma[] = '0';
			}
            $account_res = $this->db->select('account_id,account_name,account_group_id,currency_id')->from("account")->where('created_by',$company_id)->where_in('account_id',$allowed_account_ids_comma)->get()->result();
        }else{
        ...
        if (!empty($account_res)) {
            foreach ($account_res as $account_row) {
                ...
                if ($acc_balance >= 0) {
                    ...
                    $cr_account_arr[] = array(
                        "account_id" => $account_row->account_id,
                        "account_name" => '<a href="' . base_url("report/ledger/" . $account_row->account_id) . '"><div>' . $account_row->account_name . ' ('.$curr_name.')</div></a>',
                        "amount" => '-'.abs($acc_balance)
                    );
                }
            }
        ...
        for ($i = 0; $i < $for_loop_limit; $i++) {
            ...
            if (isset($cr_account_arr[$i]['account_name'])) {
                $row[] = "<b style='font-weight: 600; font-size: 16px;'>".$cr_account_arr[$i]['account_name']."</b>";
                $row[] = '<b class="red-text">'.number_format((float) $cr_account_arr[$i]['amount'], 2, '.', '').'</b>';
    ...
    function trial_balance_datatable_2()
    {
        $group_id = '';
        ...
        for ($i = 0; $i < $for_loop_limit; $i++) {
            ...
            if (isset($dr_account_arr[$i]['account_name'])) {
                $row[] = "<b style='font-weight: 600; font-size: 16px;'>".$dr_account_arr[$i]['account_name']."</b>";
                $row[] = '<b class="blue-text">'.number_format((float) $dr_account_arr[$i]['amount'], 2, '.', '').'</b>';
            } else {
                $row[] = '';
                $row[] = '';
            }
...
//        $is_kasar_account = $this->crud->get_val_by_id('account',$account_id,'account_id','is_kasar_account');
    ...
    function save_image()
    {
        ...
        $data[] = $total2;
        $ledger_data1['data'] = array($data);
        
        $html = $this->load->view('report/ledger_report_saveimg',$ledger_data1, true);
        
        $imageWidth = 1250;
        foreach($data as $items) {
            $imageHeight += 60;
        }
        // $imageHeight = 40*$count;
        $image = imagecreatetruecolor($imageWidth, $imageHeight);
        
        // Define colors for the table elements
        $backgroundColor = imagecolorallocate($image, 255, 255, 255); // white
        $textColor = imagecolorallocate($image, 0, 0, 0); // black
        $tableLineColor = imagecolorallocate($image, 0, 0, 0); // black

        // Fill the image background with the specified color
        imagefilledrectangle($image, 10, 10, $imageWidth, $imageHeight, $backgroundColor);

        // Set the font size for the table text
        $fontSize = 12;
        $fontheadding = 16;
        ...
        $dom->loadHTML($html);
        $currency_master_row = $this->crud->get_data_row_by_where('currency_master', array('id' => $user_default_currency_id));
        $account_row = $this->crud->get_data_row_by_where('account', array('account_id' => $account_id));
        $account_name = $account_row->account_name;
        $currency_name = $currency_master_row->currency_name;
        ...
        $first = '  Date       From :          To :          ';
        $balanceString = '  Balance : '.$total;
        $second = '         '.$_POST['daterange_1'].'      '.$_POST['daterange_2'];
        $third = '  Account    '.$account_name.'           '.$currency_name.'';
        
        imagestring($image, $fontheadding, 15, 20, $first, $textColor);
        imagestring($image, $fontSize, 50, 50, $second, $textColor);
        if($total < 0){
            imagestring($image, $fontheadding, 15, 80, $balanceString, imagecolorallocate($image, 255, 0, 0));
        }else{
            imagestring($image, $fontheadding, 15, 80, $balanceString, imagecolorallocate($image, 0, 0, 255));
        } 
        imagestring($image, $fontSize, 15, 110, $third, $textColor);


        foreach ($table->getElementsByTagName('tr') as $row) {
            $x = 30; // Reset the X position for each row
            $currentColumnNo = 1;
        
            foreach ($row->getElementsByTagName('td') as $cell) {
                $cellText = $cell->nodeValue;
        
                if($currentColumnNo == 7)
                  imagestring($image, $fontSize, $x, $y, $cellText, imagecolorallocate($image, 0, 0, 255));
                else if($currentColumnNo == 8)
                  imagestring($image, $fontSize, $x, $y, $cellText, imagecolorallocate($image, 255, 0, 0));
                else if($currentColumnNo == 9)
                {
                  if($cellText < 0)
                    imagestring($image, $fontSize, $x, $y, $cellText, imagecolorallocate($image, 255, 0, 0));  
                  else
                  ...
                  imagestring($image, $fontSize, $x, $y, $cellText, imagecolorallocate($image, 0, 0, 0));
        
                $x += 140; // Adjust the column width as needed
                $currentColumnNo++;
            }
        
            $y += 40; // Adjust the row height as needed
        }
        
        $path = FCPATH . LEDGER_IMAGE;

        $imageName =  $account_name.".png";
        $outputImagePath =  $path.$imageName;

        imagepng($image, $outputImagePath);
        // Set the HTTP header to display the image as PNG
        header('Content-Type: image/png');
        // Send the image as a downloadable file
        header('Content-Disposition: attachment; filename="'.$imageName.'"');
        // Clean up the memory
        echo json_encode(['status' => "success", 'msg' => "Download", 'image' => $imageName]);
        exit();
