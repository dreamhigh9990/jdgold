...
class Account extends CI_Controller {
    ...
    function save_account() {
        ...
        $post_data['full_name'] = isset($post_data['full_name']) ? $post_data['full_name'] : null;
        $post_data['ref_by'] = isset($post_data['ref_by']) ? $post_data['ref_by'] : null;
        ...
        $post_data['currency_id'] = isset($post_data['currency_id']) ? $post_data['currency_id'] : null;
        ...
        if (isset($post_data['account_id']) && !empty($post_data['account_id']) && $post_data['account_id'] != 0) {
            ...
            if (!empty($_FILES['profile_img']['name'][0])) {
                $path = FCPATH . ACCOUNT_IMAGES . $post_data['account_id'];
                if(!file_exists($path)){
                    mkdir($path, 0777, true);
                } else { $path.='/' ; }
                if($this->upload_files($path, $post_data['profile'], $_FILES['profile_img'])===FALSE){
            ...
            if(!empty($_FILES['profile_img']['name'])){
                ...
                $post_data['profile'] = $_FILES['profile_img']['name'];
            }
            ...
            if (!empty($_FILES['profile_img']['name'][0])) {
                $path = FCPATH . ACCOUNT_IMAGES . $account_id;
                if(!file_exists($path)){
                    mkdir($path, 0777, true);
                } else { $path.='/' ; }
                if($this->upload_files($path, $post_data['profile'], $_FILES['profile_img'])===FALSE){
            ...
            if(!empty($account_id)){
                $profile_image['profile'] = $_FILES['profile_img']['name'];
                $where_array['account_id'] = $account_id;
                $result = $this->crud->update('account', $profile_image, $where_array);
            }
            
            if ($result) {
                $returns['success'] = "Added";

                $this->session->set_flashdata('success', true);
                $this->session->set_flashdata('message', 'Account Added Successfully');
                $last_query_id = $this->db->insert_id();
            } else {
                $returns['error'] = "errorAdded";
            }
        }
    ...
    function account_list() {
        if ($this->applib->have_access_role(MASTER_ACCOUNT_COMPANY_ID, "view") || $this->applib->have_access_role(MASTER_ACCOUNT_COMPANY_ID, "add") || $this->applib->have_access_role(MASTER_ACCOUNT_COMPANY_ID, "edit") || $this->applib->have_access_role(MASTER_ACCOUNT_COMPANY_ID, "delete")){
            $data = array();
    ...
    function account_datatable() {
        ...
        $config['select'] = 'a.account_id,a.ib_account_id, a.account_name,a.full_name,a.ref_by, a.account_email_ids, a.account_phone, ag.account_group_name as ag_name,a.opening_balance,a.credit_debit';
        ...
        foreach ($list as $accounts) {
            if($accounts->ib_account_id == NULL && $accounts->ib_account_id == 0){
                $row = array();
                $action = '';
